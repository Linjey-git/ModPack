package com.linjey.modpack.block;

import com.linjey.modpack.ModPackMod;
import com.linjey.modpack.block.custom.LightningInfusionerBlock;
import com.linjey.modpack.group.ModItemGroup;
import com.linjey.modpack.item.ModItems;
import net.minecraft.block.AbstractBlock;
import net.minecraft.block.Block;
import net.minecraft.block.material.Material;
import net.minecraft.item.BlockItem;
import net.minecraft.item.Item;
import net.minecraftforge.common.ToolType;
import net.minecraftforge.eventbus.api.IEventBus;
import net.minecraftforge.fml.RegistryObject;
import net.minecraftforge.registries.DeferredRegister;
import net.minecraftforge.registries.ForgeRegistries;

import java.util.function.Supplier;

public class ModBlocks {

    public static final DeferredRegister<Block> BLOCKS
            = DeferredRegister.create(ForgeRegistries.BLOCKS, ModPackMod.MOD_ID);

    public static final RegistryObject<Block> FIRE_ORE = registerBlock("fire_ore",
            () -> new Block(AbstractBlock.Properties.create(Material.ROCK)
                    .setLightLevel(value -> 10)
                    .harvestLevel(2).harvestTool(ToolType.PICKAXE)
                    .setRequiresTool().hardnessAndResistance(5.0f)));
    public static final RegistryObject<Block> WATER_ORE = registerBlock("water_ore",
            () -> new Block(AbstractBlock.Properties.create(Material.ROCK)
                    .setLightLevel(value -> 10)
                    .harvestLevel(2).harvestTool(ToolType.PICKAXE)
                    .setRequiresTool().hardnessAndResistance(5.0f)));
    public static final RegistryObject<Block> WIND_ORE = registerBlock("wind_ore",
            () -> new Block(AbstractBlock.Properties.create(Material.ROCK)
                    .setLightLevel(value -> 10)
                    .harvestLevel(2).harvestTool(ToolType.PICKAXE)
                    .setRequiresTool().hardnessAndResistance(5.0f)));
    public static final RegistryObject<Block> EARTH_ORE = registerBlock("earth_ore",
            () -> new Block(AbstractBlock.Properties.create(Material.ROCK)
                    .setLightLevel(value -> 10)
                    .harvestLevel(2).harvestTool(ToolType.PICKAXE)
                    .setRequiresTool().hardnessAndResistance(5.0f)));


    public static final RegistryObject<Block> LIGHTNING_INFUSIONER = registerBlock("lightning_infusioner",
            () -> new LightningInfusionerBlock(AbstractBlock.Properties.create(Material.IRON)
                    .harvestLevel(2).harvestTool(ToolType.PICKAXE)
                    .setRequiresTool().hardnessAndResistance(5.0f)));


    private static <T extends Block> RegistryObject<T> registerBlock(String name, Supplier<T> block) {
        RegistryObject<T> toReturn = BLOCKS.register(name, block);
        registerBlockItem(name, toReturn);
        return toReturn;
    }

    private static <T extends Block> void registerBlockItem(String name, RegistryObject<T> block) {
        ModItems.ITEMS.register(name, () -> new BlockItem(block.get(),
                new Item.Properties().group(ModItemGroup.MODPACK_BLOCKS_GROUP)));
    }

    public static void register(IEventBus eventBus) {
        BLOCKS.register(eventBus);
    }

}
